...
@router.get("/{book_id}")
async def get_book_by_id(book_id: int, current_user: User = Depends(get_current_user)) -> BookResponseSchema:
...
async def delete_book_by_id(book_id: int, current_user: User = Depends(get_current_user)):
    try:
...
async def update_book_by_id(book_id: int, data: BookUpdateSchema, current_user: User = Depends(get_current_user)):
    try:
        await BookDAO.update_by_id(book_id, **data.model_dump())
        return {
            'status': 'ok',
            'message': 'success'
        }
